...
class Landing extends Component {
    ...
    //toggles login modal in and out of view
    handletoggle = () => {
    ...
    //handles a scroll event listener that only fires if the app is not in mobile view
    setScroll = () => {
    ...
    // calls a scroll event for the nav bar when mounting
    componentDidMount() {
    ...
    // thakes an event and a call back so that the call back can reach the context API for global state mgmt
    handleSubmit = (e, cb) => {
        e.preventDefault();
        const data = {
            ...
            <Consumer>
                {context =>
                    <div ref={this.loginRef} className="landingCont">
                        ...
                        {
                            //checks to see the user is authorized
